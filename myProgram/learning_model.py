...
with open(TRAIN_FILE_NAME, 'r') as in_file:
	...
	for line in train_set:
		line = line.split()[2:] # skip the formula identifier
		try:
...
with open(TEST_FILE_NAME, 'r') as in_file:
	train_set = in_file.readlines()
	for line in train_set[: int(0.2 * len(train_set))]:
		line = line.split()[2:]
		try:
			X.append(map(float, line[:-1]))
			Y.append(float(line[-1]))
			...
			print line
	for line in train_set[int(0.2 * len(train_set)):-int(0.4 * len(train_set))]:
		line = line.split()[2:]
		try:
			...
			print line
	for line in train_set[-int(0.4 * len(train_set)):]:
		line = line.split()[2:]
		try:
