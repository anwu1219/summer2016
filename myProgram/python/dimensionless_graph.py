...
def main():
    ...
    features += add_stat_d(VIG.degree().values(), num_vars) # VIG degree features
    # print "4-8 VCG var degree features", add_stat(VCG.degree().values()[:num_vars])
    features += add_stat_d(VCG.degree().values()[:num_vars], num_vars)  # VCG var degree features
    # print "9-13 VCG clause degree features", add_stat(VCG.degree().values()[num_vars:])
    features += add_stat_d(VCG.degree().values()[num_vars:], num_vars)  # VCG clause degree features
...
def add_stat(lst, num_vars):
    """                                                                         
    Normalize the lst with the number of variables, add max, min, mean, std of the give statistics to the features list.        
    """
    for i in range(len(lst)):
