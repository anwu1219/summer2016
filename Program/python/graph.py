...
import timeit
"""
This file calculates the features of a given .dimacs file and stores them in a 
.txt file.
"""
def main():
    source = sys.argv[1]
    ...
    for i in range(num_vars+1)[1:]:
        new_formula_pos = trim_formula(formula, i)
        new_formula_neg = trim_formula(formula, -i)
        write_to_features(new_formula_pos, new_formula_neg, features_all, i, num_vars)
    features_all = preprocessing.scale(features_all)
    
    #Computing variable features
    degree = nx.degree_centrality(VG)
    closeness = nx.closeness_centrality(VG)
    betweenness = nx.betweenness_centrality(VG)
    eigen = nx.eigenvector_centrality(VG)
    commun = nx.communicability_centrality(VG)
    centrality_features = np.empty([num_vars, 5])
    for i in range(num_vars):
        centrality_features[i,0] = degree[i+1]
        centrality_features[i,1] = closeness[i+1]
        centrality_features[i,2] = betweenness[i+1]
        centrality_features[i,3] = eigen[i+1]
